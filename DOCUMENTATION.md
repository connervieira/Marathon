...
2. After creating an adming account, navigate to the login page if you aren't signed in automatically.
    - Example: `http://localhost/marathon/login.php`
    - Note: Make sure you are on the Admin Login Page, and not the Employee Login Page.
...
4. Make sure 'Disable Admin Sign Ups' is checked.
    - The 'Disable Admin Sign Ups' settings prevents new admin accounts from being generated. If this setting is turned off, anyone with access to your Marathon instance will be able to make an admin account and modify your Marathon instance.
...
6. Enter the currency your business uses in the "Business Currency Symbol" field.
    ...
    - This setting doesn't have a big effect on the underlying system of Marathon. It's simply a conveinence setting that helps determine things like rounding. For example, if this value is set to 'USD', values will typically be rounded off to two decimal places.
7. After you've configured the settings explained above, click 'Submit' to save configuration changes.
8. The base configuration of Marathon is now complete. You can now begin adding employees and positions to your Marathon instance.
